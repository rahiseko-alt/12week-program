...
- 根因：①最初のコミット（`4cfb9d0`）でCIが緑になった時点でPR#13が先にマージされ、その後の2コミットは
  **PRが既にmerged/closedになった後に**同じブランチへpushされたため、mainに取り込まれなかった（auto-mergeが
  複数コミットの中から特定のコミットを選んだ、という内部動作までは未検証。確認できているのはタイミングの前後関係のみ）。
  ②pushが成功した＝mainに反映された、と暗黙に仮定し、PRのマージ後に`pull_request_read`でhead shaとmainの
  実差分を照合していなかった。
