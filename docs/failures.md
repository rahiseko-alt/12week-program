...
## 2026-07-27 本番デプロイが2日以上古いまま気づかれず、公開スモークが「/」だけを見ていて検知できなかった
- 事象：D3（アプリが公開URLで誰でも開ける）の検証のため、`prod-smoke` に概算アプリのルート
  `/estimate` への到達チェックを追加したところ、`https://cc-v2-web.vercel.app/` は 200 を返すのに
  `/estimate` は **404** だった（run 30231324969）。`/estimate` は 2026-07-25 の PR #13 で `main` に
  マージ済みであり、本番デプロイがそれ以前（7/24 頃）の古いビルドのまま止まっていたことが判明した。
  PR #32 のコミットステータスにも Vercel のデプロイステータスが1件も無く、Git 連携が本番へ自動デプロイ
  していない状態だった。
- 根因：既存の `prod-smoke`（G-2-2）が **トップページ「/」1本しか叩いていなかった**。トップページは
  古いビルドでも同じ内容を返すため、「本番が生きている」ことは示せても「**本番が main の内容である**」
  ことは一切示していなかった。この盲点により、デプロイ停止が2日以上検知されなかった。
- 対処：`prod-smoke` に `/estimate` の 200＋マーカーチェックを追加し、D3 の verify を機械が実行できる
  状態にした。本番デプロイの復旧はマスターが Vercel ダッシュボードで Git 連携を再接続して実施する。
- 教訓：**公開到達のスモークは「代表1ページが 200」で満足してはならない。受入対象となるルートごとに
  個別に叩く。** 特に「サイトが生きているか」と「サイトが最新のコードか」は独立して落ちうる別の事実で
  あり、前者だけを見る監視は後者の障害（デプロイ停止・連携切れ）を構造的に見逃す。新しいページ・機能を
  main にマージしたら、その公開到達チェックも同時にスモークへ足すこと。
