...
- **報告・説明は抽象論で済ませない（マスター指示・2026-07-28）**：「理論を採用した」「原則を適用した」等の
  一般論で終わらせず、必ず**具体例**（誰が・何を・どんな数値/固有名詞/シナリオで）とセットで示す。
  理論やルールを説明するときは、それが実際にどのノード・どの成果物・どの具体的ケースに適用されたかを
  名指しできる粒度まで踏み込む（例：「DP-1原子分解を適用した」ではなく「B1のコマ2行『ベースライン実測値と
  目標値が確定している』という1文がDP-1の適用例」まで書く）。
